...
namespace e8086UnitTests
{
    ...
    public class EUTests_ADD
    {
        ...
        public void Test00()
        {
            ...
            cpu.NextInstruction();
            Assert.AreEqual(0x00, cpu.Bus.GetData(0, 0x0115), "ADD (4) result failed");
        ...
        {
            // Test Flags
            CPU cpu = GetCPU(new byte[] { 0x01, 0xd8, /* ADD AX,BX */
                                      0x01, 0xd1, /* ADD CX,DD */
                                      0x01, 0xe7, /* ADD DI,SP */
            ...
            cpu.NextInstruction();
            Assert.AreEqual(0x0000, cpu.Bus.GetData(1, 0x0115), "ADD (4) result failed");
            Assert.AreEqual(true, cpu.EU.CondReg.CarryFlag, "ADD (4) carry flag failed");
            Assert.AreEqual(true, cpu.EU.CondReg.ParityFlag, "ADD (4) parity flag failed");
            Assert.AreEqual(true, cpu.EU.CondReg.ZeroFlag, "ADD (4) zero flag failed");
            Assert.AreEqual(false, cpu.EU.CondReg.SignFlag, "ADD (4) sign flag failed");
        ...
        {
            // Test Flags
            CPU cpu = GetCPU(new byte[] { 0x02, 0xd8, /* ADD BL,AL */
                                      0x02, 0xd1, /* ADD DL,CL */
                                      0x02, 0xe7, /* ADD AH,BH */
        ...
        {
            // Test Flags
            CPU cpu = GetCPU(new byte[] { 0x03, 0xd8, /* ADD BX,AX */
                                      0x03, 0xd1, /* ADD DX,CX */
                                      0x03, 0xe7, /* ADD SP,DI */
        ...
        {
            // Test Flags
            CPU cpu = GetCPU(new byte[] { 0x04, 0x28, /* ADD AL,28H */
        ...
        {
            // Test Flags
            CPU cpu = GetCPU(new byte[] { 0x05, 0x00, 0x28, /* ADD AX,2800H */
        ...
        public void Test80()
        {
            CPU cpu = GetCPU(new byte[] { 0x80, 0xc1, 0x80 });
            ...
            cpu.Bus.SaveData(0, 0x0115, 0x80);  // 80+80=100.  Carry=1, Parity=1, Zero=1, Sign=0, AuxCarry=0, Overflow=1
            cpu.NextInstruction();
            Assert.AreEqual(0x00, cpu.Bus.GetData(0, 0x0115), "ADD (2) result failed");
            Assert.AreEqual(true, cpu.EU.CondReg.CarryFlag, "ADD (2) carry flag failed");
            Assert.AreEqual(true, cpu.EU.CondReg.ParityFlag, "ADD (2) parity flag failed");
            Assert.AreEqual(true, cpu.EU.CondReg.ZeroFlag, "ADD (2) zero flag failed");
            Assert.AreEqual(false, cpu.EU.CondReg.SignFlag, "ADD (2) sign flag failed");
            ...
            cpu.Bus.SaveData(1, 0x0115, 0x1234);  // 1234+2e80=40b4.  Carry=0, Parity=0, Zero=0, Sign=0, AuxCarry=0, Overflow=0
            cpu.NextInstruction();
            Assert.AreEqual(0x40b4, cpu.Bus.GetData(1, 0x0115), "ADD (2) result failed");
